...
from scripts.algo.genetic.select import select_2dim_nom, select_3dim_nom
...
def gen_alg_3dim(mu,sigma,N,population_size = DEFAULT_POP_SIZE,selection_size = 20,max_iteration = DEFAULT_MAX_ITER,selection = "nom",trace = False):
    """
        genetic algo to generate some weights
    ...
    while stagnate == False and i < max_iteration:
        ...
        if trace:
            print("iteration -",i)
            print("selection - ",end="")
        selection = select_3dim_nom(population,get_yield,get_risk,get_trans_cost,mu,sigma,base_w,10)
        if trace:
            print(len(selection),"-",len(selection[0]))
        if selection == selection_nm1 and selection_nm1 == selection_nm2:
            stagnate = True
